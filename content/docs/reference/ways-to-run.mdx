...
> **Статус перевода:** исходник. Первоисточник: [Orca Docs / Ways to run Orca](https://www.onorca.dev/docs/ways-to-run).
...
Day-to-day coding, fast iteration

**SSH target**

A remote host you connect to over SSH

You (or your team)

Dev boxes, GPU hosts, always-on VPS

**Remote Orca Server**

A machine running Orca desktop or `orca serve`

You (or your team)

Persistent shared runtime, mobile, automation

**Per-workspace environment**

A disposable VM/sandbox per workspace

Your cloud account (BYO provider)

Isolated, ephemeral agent compute
...
Remote machine (Orca desktop or `orca serve`)

Disconnect

Agents keep running on the host; laptop reattaches

Full session state lives on the server

Multi-client

One laptop drives the host

Laptop, web, mobile, and automation can share the same runtime

Typical setup

Import SSH config, pick **Run on**

Share the server app or run `orca serve`, then pair with a URL
...
![Settings → Experimental → Per-Workspace Environments — enable the skill, then have an agent set up a recipe for the repo.](https://www.onorca.dev/docs/ways-to-run-per-workspace-env.png)
